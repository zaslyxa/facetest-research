...
function showCsvDownloadPrompt() {
  ...
  els.finishSummary.textContent = `Данные пока не подтвердились в базе. Не закрывайте вкладку, пока не скачаете файл. ${FINISH_DOWNLOAD_PROMPT} ${FINISH_DOWNLOAD_DETAILS}`;
  els.downloadCsvButton.textContent = "Скачать CSV";
  els.downloadCsvButton.classList.remove("hidden");
}

function showSavingResultsMessage() {
  els.finishTitle.textContent = "Не закрывайте вкладку";
  els.finishSummary.textContent = FINISH_SAVING_PROMPT;
  els.downloadCsvButton.textContent = "Скачать CSV";
  els.downloadCsvButton.classList.remove("hidden");
}
...
function isDuplicateResponseError(message) {
  try {
    const error = JSON.parse(message);
...
async function saveToSupabase(rows, { requestAttempts = config.supabaseRequestAttempts, onRetry } = {}) {
  ...
  try {
    const preparedRows = rows.map(withResponseId);
    const response = await fetchWithRetry(endpoint, {
      method: "POST",
      headers: buildSupabaseHeaders(),
    ...
    if (!response.ok) {
      const message = await response.text();
      if (isDuplicateResponseError(message)) {
        if (preparedRows.length === 1) return { ok: true };
        return saveResponseRowsIndividually(endpoint, preparedRows, { requestAttempts, onRetry });
      }
...
async function saveResponseRowsIndividually(endpoint, rows, { requestAttempts, onRetry }) {
  for (const row of rows) {
    ...
    const message = await response.text();
    if (isDuplicateResponseError(message)) continue;
